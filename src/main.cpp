...
static const int64 nTargetTimespan_V1 =  6 * 60 * 3 * 20; //  >6 hours;
static const int64 nTargetTimespan_V2 =  5 * 90; //  >7.5 minutes (or 5 blocks);
static const int64 nTargetSpacing = 1 * 90; //  1.5 minutes
static const int64 nInterval = nTargetTimespan_V1 / nTargetSpacing;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlock *pblock, int64 nTT, int64 nI)
{
    ...
    if ((pindexLast->nHeight+1) % nI != 0)
    {
        ...
        if (fTestNet)
        {
            ...
            else
            {
                ...
                while (pindex->pprev && pindex->nHeight % nI != 0 && pindex->nBits == nProofOfWorkLimit)
                    pindex = pindex->pprev;
                return pindex->nBits;
            }
        }
    ...
    int blockstogoback = nI-1;
    if ((pindexLast->nHeight+1) != nI)
        blockstogoback = nI;
    ...
    if (nActualTimespan < nTT/4)
        nActualTimespan = nTT/4;
    if (nActualTimespan > nTT*4)
        nActualTimespan = nTT*4;
